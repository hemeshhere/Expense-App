...
const paymentController={
    ...
    verifyOrder: async (request, response) => {
        try {
            ...
            //create unique digital footprint (HMAC) of the secret key
                .createHmac("sha256", process.env.RAZORPAY_KEY_SECRET)
            //Feed both HMAC and body into hashing functions
    ...
    createSubscription: async (request, response)=>{
        try{
            ...
            return response.status(200).json({subscription: subscription});
        }
        catch(error){
            console.log(error);
            return response.status(500).json({message: 'Internal Server Error'});

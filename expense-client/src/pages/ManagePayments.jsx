...
function ManagePayments(){
    ...
    const paymentResponseHandler = async (credits, payment) =>{
        try {
            ...
            console.log(error);
            setErrors({message: 'Unable to process payment request, contact consumer service'});
        }
    };
    ...
    const handlePayment= async (credits)=>{
        try{
            ...
            console.log(error);
            setErrors({message: 'Unable to process this payment request'});
        }
        finally{
            setLoading(false);
        }
        ...
        <div className='container p-5'>
            ...
            {CREDIT_PACKS.map((credit, index)=>(
                <div key={index} className='col-auto border m-2 p-2'>
                    ...
                    <button className='btn btn-outline-primary' onClick={()=>{ handlePayment(credit.credits);}}>Buy Now</button>
                </div>
            ))}
        </div>
    )
